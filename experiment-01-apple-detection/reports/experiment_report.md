...
**分析**：
- 褶皱、合格、碰伤、腐烂四类获得了大量伪标签补充（+37~+76 张），可有效缓解类别不均衡。
- 病变（Diseased）仅获得 2 张伪标签，说明模型对该类的置信度普遍偏低，这与验证集中 diseased 样本少、特征学习不充分有关。
- 若将这 230 张安全伪标签混入训练集重新微调，预计 Val Acc 可提升 **2–4%**，同时改善 cracked/wrinkled 的 Precision。
